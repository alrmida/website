...
USING (EXISTS (
  ...
  WHERE m.machine_id = daily_production_summary.machine_id 
    ...
    AND EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.role = 'client'::user_role)
));
...
USING (EXISTS (
  ...
  WHERE profiles.id = auth.uid() 
    AND profiles.role = ANY (ARRAY['kumulus_personnel'::user_role, 'kumulus_admin'::user_role])
));
